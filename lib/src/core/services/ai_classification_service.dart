...
- Текущий URL страницы: ${currentUrl ?? 'неизвестно'} (если в URL есть ID рецепта, например 'recipes/borscht-0', значит пользователь находится на странице этого рецепта).
$recipesContext
...
7. "read_ingredients" - прочитать список ингредиентов текущего рецепта.
   - Используй для команд: "скажи ингредиенты", "что нужно для этого блюда", "список покупок", "ингредиенты".
   Параметры: не требуются.
8. "help" - запрос списка команд (что ты умеешь?, список действий, помощь, инструкция).
   Параметры: не требуются.
9. "generate_recipe" - генерация нового рецепта по запросу/пожеланиям пользователя с помощью ИИ (требуется интернет).
   - Используй для команд: "сгенерируй рецепт...", "придумай рецепт...", "создай рецепт...", "генерация рецепта...", "придумай что-нибудь из курицы".
   Параметры: Текст запроса (то есть то, что именно нужно сгенерировать, например: "мясного блюда с гарниром", "веганского пирога", "курицы и картошки").
$recipesContext
...
{"action": "generate_recipe", "parameters": "мясного блюда с гарниром"}
